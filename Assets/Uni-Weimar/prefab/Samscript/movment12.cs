...
{
    public class movment12 : MonoBehaviour
    {
        #region Enums
        ...
        private MonoBehaviour groundFollowing;
        ...
        private void OnDisable()
        {
            ...
            // Always re-enable GroundFollowing when this script is disabled
            if (groundFollowing != null)
                groundFollowing.enabled = true;
        }
        ...
        private void Initialize()
        {
            ...
            if (anatomy == null)
            {
                Debug.LogError("movment12: AvatarHMDAnatomy not found.");
                return;
            }
            ...
            {
                Debug.LogError("movment12: head / leftHand / rightHand reference missing in AvatarHMDAnatomy.");
                return;
            }
            ...
            characterController = steeringTarget.GetComponent<CharacterController>();
            if (characterController == null)
                Debug.LogWarning("movment12: No CharacterController found on steeringTarget. Collisions will not work correctly.");

            // Cache GroundFollowing — search on steeringTarget first, then parents
            foreach (MonoBehaviour script in steeringTarget.GetComponents<MonoBehaviour>())
            {
                if (script.GetType().Name == "Groundfollowing")
                {
                    groundFollowing = script;
                    break;
                }
            }
            ...
            if (groundFollowing == null)
            {
                foreach (MonoBehaviour script in steeringTarget.GetComponentsInParent<MonoBehaviour>())
                {
                    if (script.GetType().Name == "Groundfollowing")
                    {
                        groundFollowing = script;
            ...
            if (groundFollowing == null)
                Debug.LogWarning("movment12: Groundfollowing script not found. Height conflict may occur while moving.");
        ...
        private void ApplySteering()
        {
            ...
            InputAction triggerAction = navigationHand == HandType.Left
                ? leftTrigger.action
                : rightTrigger.action;
            ...
            // Player is not moving — re-enable GroundFollowing and exit
            ...
            // Player is moving — disable GroundFollowing to prevent height conflict
            if (groundFollowing != null)
                groundFollowing.enabled = false;

            // Trigger: 0 = not pressed, 1 = fully pressed → up to 3x speed
            float triggerValue = triggerAction != null ? triggerAction.ReadValue<float>() : 0f;
            float speedMultiplier = 1f + triggerValue * 2f;
            ...
            Vector3 motion = moveDir.normalized * steeringSpeed * speedMultiplier * input.magnitude * Time.deltaTime;

            if (characterController != null)
                characterController.Move(motion);
            else
                steeringTarget.position += motion;
        }
        ...
        {
            InputAction action = navigationHand == HandType.Left
                ? leftThumbstick.action
                : rightThumbstick.action;
        ...
        private void PerformTeleport()
        {
            ...
            // Calculate XZ offset from current head position to preview target
            ...
            // Disable GroundFollowing and CharacterController for clean warp
            if (groundFollowing != null) groundFollowing.enabled = false;
            if (characterController != null) characterController.enabled = false;

            teleportationTarget.position += movement;
            ...
            // Ground check — snap rig Y to ground surface
            ...
            // Re-enable everything after teleport
            if (characterController != null) characterController.enabled = true;
            if (groundFollowing != null) groundFollowing.enabled = true;
